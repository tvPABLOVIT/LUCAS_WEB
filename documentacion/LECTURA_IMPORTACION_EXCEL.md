...
**Formato estimaciones (sN_AAAA):** la fecha que aparece en la fila 21 del Excel y el día al que se guardan los importes en el sistema llevan **14 días de desfase** (`targetDate = refDate − 14 días`). Es independiente de la navegación “semana anterior/siguiente” del dashboard (eso sigue siendo 7 días).
...
|-------|------|----------|-----------|-----|
| **Fechas de referencia (semana del Excel)** | 21 | C, D, E, F, G, H, I (3–9) | Una fecha por día (L–D) de la **semana que representa el archivo** | Se lee cada celda con `TryGetRefDateFromCell`; la **fecha de datos** es `refDate.AddDays(-14)` (**14 días** de desfase respecto a la fecha que muestra el Excel). |
...
- **Fecha “objetivo” (targetDate):** para cada columna, `refDate = TryGetRefDateFromCell(ws.Cell(21, col), ...)` y luego `targetDate = refDate.AddDays(-14)`. Es decir, hay **14 días de desfase** entre la fecha que aparece en el Excel y el día al que se asignan los importes en el sistema (**2 semanas antes**).
...
La lectura recorre **7 columnas** (C a I). Cada **columna** representa un día de la semana (Lunes a Domingo) de la **semana que representa el archivo**. Los datos se importan con **14 días de desfase** (targetDate = fecha de la fila 21 − 14 días).
