...
|-------|------|----------|-----------|-----|
| **Fechas de referencia (semana del Excel)** | 21 | C, D, E, F, G, H, I (3–9) | Una fecha por día (L–D) de la **semana que representa el archivo** | Se lee cada celda con `TryGetRefDateFromCell`; la **fecha de datos** es `refDate.AddDays(-7)` (1 semana antes). |
...
- **Fecha “objetivo” (targetDate):** para cada columna, `refDate = TryGetRefDateFromCell(ws.Cell(21, col), ...)` y luego `targetDate = refDate.AddDays(-7)`. Es decir, el Excel representa una semana y el programa importa los datos como si fueran de **una semana antes**.
...
La lectura recorre **7 columnas** (C a I). Cada **columna** representa un día de la semana (Lunes a Domingo) de la **semana que representa el archivo**. Los datos se importan como si fueran de **1 semana antes** (targetDate = fecha de la fila 21 − 7 días).
...
3. Para **col = 3** hasta **9** (C a I):  
   ...
   b. targetDate = refDate.AddDays(-7) (y excepción S1/S2 2026 → diciembre 2025).
