(function (global) {
  ...
  function render(container) {
    ...
    function load() {
      ...
      // Para operación semanal (semana actual), la comparación clave es "mismos días".
      // Aproximamos el corte de la semana pasada al mismo día de la semana que hoy (Lun=0..Dom=6).
      var dowIdx = (function () {
        try {
          var d = new Date();
          var js = d.getDay(); // Dom=0..Sáb=6
          return (js + 6) % 7; // Lun=0..Dom=6
        } catch (e) { return 6; }
      })();
      var asOfPrevTriad = isTriadCurrentWeek ? addDays(prevMon, dowIdx) : addDays(prevMon, 6);
      auth.fetchWithAuth('/api/estimaciones/comparativas?weekStart=' + encodeURIComponent(ws) + '&asOf=' + encodeURIComponent(todayYmd) + '&mode=plan').then(function (r) {
        ...
        if (realForKpiComparisons != null && data.prevWeekRevenue != null && data.prevWeekRevenue > 0) {
          ...
          var lineRev = '';
          if (pct > 0) lineRev = '<div class="kpi-card-sub kpi-card-sub--up">+' + pct.toFixed(1) + '% vs sem. ant.</div>';
          else if (pct < 0) lineRev = '<div class="kpi-card-sub kpi-card-sub--down">' + pct.toFixed(1) + '% vs sem. ant.</div>';
          else lineRev = '<div class="kpi-card-sub">0% vs sem. ant.</div>';
          pctVsPrev += lineRev;
          pctVsPrevCard += lineRev;
        }
        ...
        if (objNum > 0 && realForKpiComparisons != null) {
          ...
          var lineObj = '<div class="kpi-card-sub ' + objetivoClass + '">' + pctObjStr + '% vs fact. objetivo</div>';
          pctVsPrev += lineObj;
          pctVsPrevCard += lineObj;
        } else {
          var lineObjM = '<div class="kpi-card-sub kpi-card-sub--muted">vs fact. objetivo: —</div>';
          pctVsPrev += lineObjM;
          pctVsPrevCard += lineObjM;
        }
        ...
        if (data.avgProductivity != null && data.prevWeekProductivity != null && data.prevWeekProductivity > 0) {
          ...
          var lineP = '';
          if (pctProd > 0) lineP = '<div class="kpi-card-sub kpi-card-sub--up">+' + pctProd.toFixed(1) + '% vs sem. ant.</div>';
        ...
        if (prodObjNum > 0 && data.avgProductivity != null) {
          ...
          var linePO = '<div class="kpi-card-sub ' + prodObjClass + '">' + pctProdObjStr + '% vs prod. objetivo</div>';
          pctVsPrevProd += linePO;
          pctVsPrevProdCard += linePO;
        } else {
          var linePOM = '<div class="kpi-card-sub kpi-card-sub--muted">vs prod. objetivo: —</div>';
          pctVsPrevProd += linePOM;
          pctVsPrevProdCard += linePOM;
        }
        ...
        if (data.costePersonalPctFacturacion != null) {
          ...
          var lineCf = '<div class="kpi-card-sub ' + costeClass + '">' + pctCoste.toFixed(1) + '% vs facturación</div>';
          costeCard = lineCf;
          costeSub = lineCf;
        }
        ...
        if (isTriadCurrentWeek) {
          ...
          topKpisHtml =
            ...
            directorAccumCard('Facturación total', revValue || '—', pctVsPrevCard, 'dashboard-director-accum-badges') +
            directorAccumCard('Productividad media', prodValue || '—', pctVsPrevProdCard, 'dashboard-director-accum-badges') +
            directorAccumCard('Horas totales', (data.totalHours != null ? data.totalHours.toFixed(1) : '—'), '', '') +
            directorAccumCard('Coste personal', costeValue || '—', costeCard, 'dashboard-compare-kpi--coste dashboard-director-accum-badges') +
            '</div>';
        } else {
        ...
        if (triadEl) {
          ...
          function kpiCard(label, value, sub, cls) {
            return '<div class="dashboard-compare-kpi' + (cls ? (' ' + cls) : '') + '"><div class="label">' + escapeHtml(label) + '</div><div class="value">' + value + '</div><div class="sub">' + escapeHtml(sub || '') + '</div></div>';
          }
          ...
          if (isTriadCurrentWeek) {
            ...
            try {
              for (var i = 0; i < 7; i++) {
                var d = addDays(curMonTriad, i);
                var hasActual = rows[i] && rows[i].cur != null;
                if (hasActual && curPredByDate && curPredByDate[d] != null) predToDate += Number(curPredByDate[d] || 0);
              }
            } catch (e) { predToDate = 0; }
            var remaining = Math.max(0, predTotalCur - predToDate);
            ...
            var clsCur2 = (sameDelta >= 0) ? 'dashboard-compare-kpi--up' : 'dashboard-compare-kpi--down';
            kpiHtml += '<div class="dashboard-compare-kpi ' + clsCur2 + '"><div class="label">Seleccionada vs pasada (mismos días)</div><div class="value">' + (sameDelta >= 0 ? '+' : '−') + fmtEur(Math.abs(sameDelta)) + '</div><div class="sub">' + (samePct != null ? fmtPct(samePct) : '—') + (sameDaysCount > 0 ? (' · ' + sameDaysCount + '/7 días') : '') + '</div></div>';
          ...
          var driversHtml = '';
          if (isTriadCurrentWeek) {
            var driversVsPred = rows.filter(function (r, idx) {
              var d = addDays(curMonTriad, idx);
              return r.cur != null && curPredByDate && curPredByDate[d] != null;
            }).map(function (r, idx) {
              var d = addDays(curMonTriad, idx);
              var predDay = Number(curPredByDate[d] || 0);
              var delta = Number(r.cur || 0) - predDay;
              var pct = predDay > 0 ? (delta / predDay) * 100 : null;
              return { day: r.day, delta: delta, pct: pct };
          ...
          var tableRowsTriad = rows.map(function (r, idx) {
            ...
            if (isTriadCurrentWeek) {
              var dateCur = addDays(curMonTriad, idx);
              var predDay = (curPredByDate && curPredByDate[dateCur] != null) ? Number(curPredByDate[dateCur]) : null;
              var dVsPred = (r.cur != null && predDay != null) ? (r.cur - predDay) : null;
              var pVsPred = (r.cur != null && predDay != null && predDay > 0) ? (dVsPred / predDay) * 100 : null;
              return '<tr class="' + (r.pending ? 'dashboard-triad-row--pending' : '') + '">' +
                ...
                '<td>' + (predDay != null ? fmtEur(predDay) : '<span class="dashboard-triad-muted">—</span>') + '</td>' +
                '<td>' + cellDelta(r.dCur, r.pCur) + '</td>' +
                '<td>' + cellDelta(dVsPred, pVsPred) + '</td>' +
                '</tr>';
            }
            return '<tr class="' + (r.pending ? 'dashboard-triad-row--pending' : '') + '">' +
              '<td class="dashboard-compare-td-day"><div class="dashboard-compare-day">' + r.day + '</div></td>' +
          ...
          triadEl.innerHTML =
            ...
            (isTriadCurrentWeek
              ? '<thead><tr><th>Día</th><th>Semana pasada (real)</th><th>Semana actual (real)</th><th>Pred. semana actual</th><th>Δ Actual vs pasada</th><th>Δ Actual vs pred.</th></tr></thead>'
          ...
          // Acciones: scroll a importar (bloque al final del dashboard)
          try {
    ...
    // Importación (Excel/PDF) en Dashboard (abajo del todo)
      ...
      function sendNext(index) {
        if (index >= total) {
          ...
          if (allErrors.length > 0) {
            ...
            if (excelStatus) excelStatus.className = 'dashboard-import-status error';
          } else {
            if (excelStatus) excelStatus.className = 'dashboard-import-status success';
          }
          if (excelStatus) excelStatus.textContent = msg;
    ...
    if (pdfFile) pdfFile.addEventListener('change', function () {
      ...
      if (pdfStatus) { pdfStatus.textContent = 'Enviando…'; pdfStatus.className = 'dashboard-import-status'; }
      ...
      auth.fetchWithAuth('/api/import/cuadrante-pdf?weekStart=' + encodeURIComponent(ws), { method: 'POST', body: fd }).then(function (r) {
        if (r.status === 401) {
          if (global.LUCAS_APP && global.LUCAS_APP.onUnauthorized) global.LUCAS_APP.onUnauthorized();
          if (pdfStatus) { pdfStatus.textContent = 'Sesión expirada.'; pdfStatus.className = 'dashboard-import-status error'; }
          return null;
        }
        if (!r.ok) {
