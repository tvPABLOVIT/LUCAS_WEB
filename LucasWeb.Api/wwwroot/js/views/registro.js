(function (global) {
  ...
  function getFormHtml() {
    ...
    return '<div class="registro-card">' +
      ...
      (s && (s.revenue_per_waiter_sala != null || s.difficulty_score != null) ? '<p class="registro-comfort-hint" title="Facturación por persona en sala y dificultad del turno (1–5)">' + (s.revenue_per_waiter_sala != null ? 'Facturación por personal de sala: ' + Number(s.revenue_per_waiter_sala).toFixed(0) + ' €' : '') + (s.revenue_per_waiter_sala != null && s.difficulty_score != null ? ' · ' : '') + (s.difficulty_score != null ? ' Dificultad: ' + Number(s.difficulty_score).toFixed(1) + '/5' : '') + (s.comfort_level ? ' · ' + s.comfort_level : '') + '</p>' : '') +
      (s && (s.revenue_per_waiter_cocina != null || s.difficulty_score_kitchen != null) ? '<p class="registro-comfort-hint" title="Facturación por persona en cocina y dificultad cocina (Q5)">' + (s.revenue_per_waiter_cocina != null ? 'Facturación por personal de cocina: ' + Number(s.revenue_per_waiter_cocina).toFixed(0) + ' €' : '') + (s.revenue_per_waiter_cocina != null && s.difficulty_score_kitchen != null ? ' · ' : '') + (s.difficulty_score_kitchen != null ? ' Dificultad cocina: ' + Number(s.difficulty_score_kitchen).toFixed(1) + '/5' : '') + (s.comfort_level_kitchen ? ' · ' + s.comfort_level_kitchen : '') + '</p>' : '') +
      '</div></div>' +
